    ...
    try {
      switch (endpoint) {
        ...
        case 'sessionModel': {
          ...
          if (!agentDefaultModel) return err('no-agent-model', 'agentDefaultModel service unavailable');
          const sel = agentDefaultModel.currentSelection();
          if (!sel || !sel.model) return err('no-model', 'no current model selected');
        ...
        case 'optimize.poll': {
          ...
          if (!entry) return err('no-task', 'unknown task');
